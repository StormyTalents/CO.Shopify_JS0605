...
import {  dl_sign_up } from '@/lib/dataLayer'
import { setActivateCustomer } from '@/utils/helpers'
...
function Page({vipDiscount}) {
  ...
  const [dacu, setDacu] = useState(null)
  ...
  async function signUpCustomer(e) {
    ...
    const activate = await setActivateCustomer(dacu,pass)
    ...
    if(activate.status == "error") {
      ...
      dl_sign_up(activate.data.customer.id, activate.data.customer.email)
    ...
    return (
        <div className="page page-width activate--page">
          <SEO title="Account Activate Page" description="Account Activate Page of Aftco"/> 
        <div className="customer-form-gird container">
          <div className="customer activate">
            <div className={`login`}>
            <h3 id="login" tabIndex="-1">Activate Your Account</h3>
            {error?
            ...
            :""}
                ...
                <div className='button--with--loading'>
                  ...
                  <button className='button' onClick={e => signUpCustomer(e)}>Activate</button>
                </div>
            </div>
          </div>
        </div>
